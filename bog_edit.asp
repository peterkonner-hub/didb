...
<%
	...
	if Request("Action") = "Ændre" then
		...
		bid = Request.Form("ibid")
		rs.Open "SELECT * FROM tbBog WHERE BogID = " & bid, Conn, adOpenStatic, adLockPessimistic
		...
		rs.Close
		set rs = Nothing
		if Session.Contents("jumpback") > "" then
			response.redirect(Session.Contents("jumpback"))
		else
		...
		if (rs.BOF AND rs.EOF) OR (bid = 0) then
			'### Record not found, close connection...
			rs.Close
			set rs = Nothing
			Conn.Close
			set Conn = Nothing
			...
			'### Fyld felter med værdier fra databasen
			fbid = rs("BogID")
			fopretdato = fFormatDateDB2Frm(rs("OpretDato"))
			faendretdato = fFormatDateDate2Frm(Date())
			fslettedato = fFormatDateDB2Frm(rs("SletteDato"))
			frkflg = rs("RkFlg")
			ftitel = rs("Titel")
			fbeskrivelse = rs("Beskrivelse")
			ffontsizeeditor = rs("FontSizeEditor")
			ffontsizescreen = rs("FontSizeScreen")
			ffontsizeprint = rs("FontSizePrint")
			ffontfaceeditor = rs("FontFaceEditor")
			ffontfacescreen = rs("FontFaceScreen")
			ffontfaceprint = rs("FontFacePrint")
%>
...
<div class="w3-grid" style="grid-template-columns:20% 60% 20%">
	...
	<div class="w3-padding">
		<h2>Ændre bog</h2>
		<p>Her kan du ændre titel og beskrivelse på din bog.<br></p>
...
-->
			<input type="hidden" name="ibid" value="<%=fbid%>">
			...
			<button type="submit" class="w3-button w3-green w3-quarter" name="Action" value="Ændre">Ændre bog</button>
