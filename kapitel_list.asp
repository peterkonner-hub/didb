...
<div class="w3-container">
	<h2>Kapitler for <u><%=Session.Contents("bogtitel")%></u></h2>
</div>

<br>

<div class="w3-container">
	<a href="kapitel_opret.asp" class="w3-button w3-green" alt="Opret nyt punkt">Opret nyt kapitel</a>
</div>
...
<%
	strSQL = "SELECT MIN(RkFlg) as MinRkFlg, MAX(RkFlg) as MaxRkFlg FROM tbKapitel WHERE (BogID = " & Session.Contents("bogid") & ") "
	set rs = Conn.Execute(strSQL)
	MinRkFlg = rs("MinRkFlg")
	MaxRkFlg = rs("MaxRkFlg")
	strSQL = "SELECT t3.*, t9.StatusText, t9.KlarmeldtJN FROM tbKapitel t3 LEFT JOIN tbStatus t9 ON (t3.StatusID = t9.StatusID) WHERE "
	strSQL = strSQL & "(t3.BogID = " & Session.Contents("bogid") & ") AND (t3.BrugerID = " & Session.Contents("userid") & ") ORDER BY t3.RkFlg "
	set rs = Conn.Execute(strSQL)
